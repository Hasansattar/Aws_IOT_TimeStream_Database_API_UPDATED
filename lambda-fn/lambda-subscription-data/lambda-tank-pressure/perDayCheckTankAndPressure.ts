...
//  console.log(`Interval Expression:---------- ${intervalExpression}`);
  ...
  const queryStringX04 = `
    ...
    WHERE time between ago(1d) and now() AND port='x04' AND Flow_Lpmin > 1
    GROUP BY port
  `;
  ...
  const queryStringX08 = `
    SELECT AVG(CAST(Flow_Lpmin AS double)) AS avg_Flow_Lpmin 
    FROM "${databaseName}"."${tableName}"
    WHERE time between ago(1d) and now() AND port='x08' AND Flow_Lpmin > 1
    GROUP BY port
  `;
  ...
  const queryStringX05 = `
    ...
    FROM "${databaseName}"."${tableName}"
    WHERE time between ago(1d) and now() AND port='x05' 
    GROUP BY port
  `;
  ...
  const queryStringX01 = `
    SELECT AVG(CAST(Pressure_bar AS double)) AS avg_Pressure_bar 
    FROM "${databaseName}"."${tableName}"
    WHERE time between ago(1d) and now() AND port='x01' 
    GROUP BY port
  `;
  ...
  const queryStringX07 = `
    SELECT AVG(CAST(Pressure_bar AS double)) AS avg_Pressure_bar 
    FROM "${databaseName}"."${tableName}"
    WHERE time between ago(1d) and now() AND port='x07'
    GROUP BY port
  `;
  ...
  try {
    ...
    console.log("waterProducttion--------",waterProduction);
     const interval="day";
    const data={
    ...
    console.log("Filter2 Result (x07 - x01):", filter2Result);
          ...
          const postData = JSON.stringify({
            query: `
              mutation UpdateTankAndPressureDataPerDay($input: UpdateTankAndPressureDataInput) {
                updateTankAndPressureDataPerDay(input: $input) {
                  currentTankLevel
                  filter1Result
                  filter2Result
                  interval
