...
export class CheckTankAndPressurePerDayLambda extends Construct {
  constructor(scope: Construct, id: string, api: GraphQLApi, db: TimestreamDatabase, role: LambdaRole) {
    ...
    const checkTankAndPressurePerDay = new lambda.Function(
      this,
      "checkTankLevelAndPressurePerDay",
      {
        runtime: lambda.Runtime.NODEJS_16_X,
        handler: "perDayCheckTankAndPressure.handler",
    ...
    const lambdaDscheckTankAndPressurePerDay = api.graphQLApi.addLambdaDataSource(
      "lambdaDscheckTankAndPressurePerDay",
      checkTankAndPressurePerDay
    );

    // Resolver for Mutation
    lambdaDscheckTankAndPressurePerDay.createResolver(
      "createResolverMutationCheckTankAndPressurePerDay",
      {
        typeName: "Mutation",
        fieldName: "updateTankAndPressureDataPerDay",
     ...
     checkTankAndPressurePerDay.addPermission(
      "EventBridgeInvokecheckTankAndPressurePerDay",
      {
    ...
    new events.Rule(this, "ruleCheckTankAndPressure", {
      schedule: events.Schedule.rate(cdk.Duration.minutes(1)),
      targets: [new targets.LambdaFunction(checkTankAndPressurePerDay)],
    });
