...
export class CheckTankAndPressurePerWeekLambda extends Construct {
  constructor(scope: Construct, id: string, api: GraphQLApi, db: TimestreamDatabase, role: LambdaRole) {
    super(scope, id);
     
    const checkTankAndPressurePerWeek = new lambda.Function(
      this,
      "checkTankLevelAndPressurePerWeek",
      {
        runtime: lambda.Runtime.NODEJS_16_X,
        handler: "perWeekCheckTankAndPressure.handler",
    ...
    const lambdaDscheckTankAndPressurePerWeek = api.graphQLApi.addLambdaDataSource(
      "lambdaDscheckTankAndPressurePerWeek",
      checkTankAndPressurePerWeek
    );
    
    // Resolver for Mutation
    lambdaDscheckTankAndPressurePerWeek.createResolver(
      "createResolverMutationCheckTankAndPressurePerWeek",
      {
        typeName: "Mutation",
        fieldName: "updateTankAndPressureDataPerWeek",
    ...
    checkTankAndPressurePerWeek.addPermission(
      "EventBridgeInvokecheckTankAndPressurePerWeek",
      {
    ...
    new events.Rule(this, "ruleCheckTankAndPressurePerWeek", {
      schedule: events.Schedule.rate(cdk.Duration.minutes(1)),
      targets: [new targets.LambdaFunction(checkTankAndPressurePerWeek)],
    });
