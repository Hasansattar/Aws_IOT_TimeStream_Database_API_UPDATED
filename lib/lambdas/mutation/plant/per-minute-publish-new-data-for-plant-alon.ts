...
export class PerMinutePublishNewDataForPlantAlonLambda  extends Construct {
  constructor(scope: Construct, id: string, api: GraphQLApi, db: TimestreamDatabase, role: LambdaRole) {
    super(scope, id);
    

    const perMinutePublishNewDataForPlantAlonLambda = new lambda.Function(this, "perMinutePublishNewDataForPlantAlonLambda", {
      runtime: lambda.Runtime.NODEJS_16_X,
      handler: "perMinutePublishNewDataForPlantAlon.handler",
      code: lambda.Code.fromAsset("lambda-fn/lambda-mutation-data/plant"),
      environment: {
    ...
    const lambdaDsPerMinutePublishNewDataForPlantAlonLambda= api.graphQLApi.addLambdaDataSource(
      "lambdaDsPerMinutePublishNewDataForPlantAlonLambda",
      perMinutePublishNewDataForPlantAlonLambda
    );
  
    // Resolver for Mutation to publish new data
    lambdaDsPerMinutePublishNewDataForPlantAlonLambda.createResolver("createResolverMutationlambdaDsPerMinutePublishNewDataForPlantAlon", {
      typeName: "Mutation",
      fieldName: "publishNewDataForPlantAlon",
    ...
    perMinutePublishNewDataForPlantAlonLambda.addPermission("EventBridgeInvoke", {
    ...
    new events.Rule(this, "rulePerMinutePublishNewDataForPlantAlonLambda", {
      schedule: events.Schedule.rate(cdk.Duration.minutes(10)),
      targets: [new targets.LambdaFunction(perMinutePublishNewDataForPlantAlonLambda)],
    });
