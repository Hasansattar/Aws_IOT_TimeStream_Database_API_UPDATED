...
export class RealTimeDataLambda extends Construct {
  constructor(scope: Construct, id: string, api: GraphQLApi, db: TimestreamDatabase, role: LambdaRole) {
    super(scope, id);

   const realTimeDataLambda = new lambda.Function(this, "RealTimeDataLambda", {
      runtime: lambda.Runtime.NODEJS_16_X,
      handler: "realTimeData.handler",
      code: lambda.Code.fromAsset("lambda-fn/lambda-mutation-data"),
      environment: {
     ...
     const lambdaDsRealTimeData = api.graphQLApi.addLambdaDataSource(
      "lambdaDsRealTime",
      realTimeDataLambda
    );

    // Resolver for Mutation to publish new data
    lambdaDsRealTimeData.createResolver("createResolverMutationpublishNewData", {
      typeName: "Mutation",
      fieldName: "publishNewData",
    ...
    realTimeDataLambda.addPermission("EventBridgeInvoke", {
    ...
    new events.Rule(this, "ruleCheckRealTimeDataLambda", {
      schedule: events.Schedule.rate(cdk.Duration.minutes(1)),
      targets: [new targets.LambdaFunction(realTimeDataLambda)],
    });
