...
export class PerMinutePublishNewDataForZoneDistributionLambda extends Construct {
  constructor(scope: Construct, id: string, api: GraphQLApi, db: TimestreamDatabase, role: LambdaRole) {
    super(scope, id);
     

    const perMinutePublishNewDataForZoneDistributionLambda = new lambda.Function(this, "perMinutePublishNewDataForZoneDistributionLambda", {
      runtime: lambda.Runtime.NODEJS_16_X,
      handler: "perMinutePublishNewDataForZoneDistribution.handler",
      code: lambda.Code.fromAsset("lambda-fn/lambda-mutation-data/zone"),
      environment: {
    ...
    const lambdaDsPerMinutePublishNewDataForZoneDistributionLambda= api.graphQLApi.addLambdaDataSource(
      "lambdaDsPerMinutePublishNewDataForZoneDistributionLambda",
      perMinutePublishNewDataForZoneDistributionLambda
    );
  
    // Resolver for Mutation to publish new data
    lambdaDsPerMinutePublishNewDataForZoneDistributionLambda.createResolver("createResolverMutationlambdaDsPerMinutePublishNewDataForZoneDistribution", {
      typeName: "Mutation",
      fieldName: "publishNewDataForZoneDistribution",
    ...
    perMinutePublishNewDataForZoneDistributionLambda.addPermission("EventBridgeInvoke", {
    ...
    new events.Rule(this, "rulePerMinutePublishNewDataForZoneDistributionLambda", {
      schedule: events.Schedule.rate(cdk.Duration.minutes(1)),
      targets: [new targets.LambdaFunction(perMinutePublishNewDataForZoneDistributionLambda)],
    });
