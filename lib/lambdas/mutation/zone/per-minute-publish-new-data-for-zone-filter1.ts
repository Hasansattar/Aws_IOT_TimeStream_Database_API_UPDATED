...
export class PerMinutePublishNewDataForZoneFilter1Lambda extends Construct {
  constructor(scope: Construct, id: string, api: GraphQLApi, db: TimestreamDatabase, role: LambdaRole) {
    super(scope, id);
     
    const perMinutePublishNewDataForZoneFilter1Lambda = new lambda.Function(this, "perMinutePublishNewDataForZoneFilter1Lambda", {
      runtime: lambda.Runtime.NODEJS_16_X,
      handler: "perMinutePublishNewDataForZoneFilter1.handler",
    ...
    const lambdaDsPerMinutePublishNewDataForZoneFilter1Lambda= api.graphQLApi.addLambdaDataSource(
      "lambdaDsPerMinutePublishNewDataForZoneFilter1Lambda",
      perMinutePublishNewDataForZoneFilter1Lambda
    );
  
    // Resolver for Mutation to publish new data
    lambdaDsPerMinutePublishNewDataForZoneFilter1Lambda.createResolver("createResolverMutationlambdaDsPerMinutePublishNewDataForZoneFilter1", {
      typeName: "Mutation",
      fieldName: "publishNewDataForZoneFilter1",
    ...
    perMinutePublishNewDataForZoneFilter1Lambda.addPermission("EventBridgeInvoke", {
    ...
    new events.Rule(this, "rulePerMinutePublishNewDataForZoneFilter1Lambda", {
      schedule: events.Schedule.rate(cdk.Duration.minutes(1)),
      targets: [new targets.LambdaFunction(perMinutePublishNewDataForZoneFilter1Lambda)],
    });
