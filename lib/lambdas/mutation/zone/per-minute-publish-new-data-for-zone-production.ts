...
export class PerMinutePublishNewDataForZoneProductionLambda extends Construct {
  constructor(scope: Construct, id: string, api: GraphQLApi, db: TimestreamDatabase, role: LambdaRole) {
    super(scope, id);

    const perMinutePublishNewDataForZoneProductionLambda = new lambda.Function(this, "perMinutePublishNewDataForZoneProductionLambda", {
      runtime: lambda.Runtime.NODEJS_16_X,
      handler: "perMinutePublishNewDataForZoneProduction.handler",
    ...
    const lambdaDsPerMinutePublishNewDataForZoneProductionLambda= api.graphQLApi.addLambdaDataSource(
      "lambdaDsPerMinutePublishNewDataForZoneProductionLambda",
      perMinutePublishNewDataForZoneProductionLambda
    );
  
    // Resolver for Mutation to publish new data
    lambdaDsPerMinutePublishNewDataForZoneProductionLambda.createResolver("createResolverMutationlambdaDsPerMinutePublishNewDataForZoneProduction", {
      typeName: "Mutation",
      fieldName: "publishNewDataForZoneProduction",
    ...
    perMinutePublishNewDataForZoneProductionLambda.addPermission("EventBridgeInvoke", {
    ...
    new events.Rule(this, "rulePerMinutePublishNewDataForZoneProductionLambda", {
      schedule: events.Schedule.rate(cdk.Duration.minutes(1)),
      targets: [new targets.LambdaFunction(perMinutePublishNewDataForZoneProductionLambda)],
    });
