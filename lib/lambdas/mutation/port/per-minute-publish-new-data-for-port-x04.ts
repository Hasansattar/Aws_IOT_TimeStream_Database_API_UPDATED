...
export class PerMinutePublishNewDataForPortX04Lambda  extends Construct {
  constructor(scope: Construct, id: string, api: GraphQLApi, db: TimestreamDatabase, role: LambdaRole) {
    super(scope, id);

    const perMinutePublishNewDataForPortX04Lambda = new lambda.Function(this, "perMinutePublishNewDataForPortX04Lambda", {
      runtime: lambda.Runtime.NODEJS_16_X,
      handler: "perMinutePublishNewDataForPort-x04.handler",
      code: lambda.Code.fromAsset("lambda-fn/lambda-mutation-data/port"),
      environment: {
    ...
    const lambdaDsPerMinutePublishNewDataForPortX04Lambda= api.graphQLApi.addLambdaDataSource(
      "lambdaDsPerMinutePublishNewDataForPortX04Lambda",
      perMinutePublishNewDataForPortX04Lambda
    );
  
    // Resolver for Mutation to publish new data
    lambdaDsPerMinutePublishNewDataForPortX04Lambda.createResolver("createResolverMutationlambdaDsPerMinutePublishNewDataForPortX04", {
      typeName: "Mutation",
      fieldName: "publishNewDataForPortx04",
    ...
    perMinutePublishNewDataForPortX04Lambda.addPermission("EventBridgeInvoke", {
    ...
    new events.Rule(this, "rulePerMinutePublishNewDataForPortX04Lambda", {
      schedule: events.Schedule.rate(cdk.Duration.minutes(1)),
      targets: [new targets.LambdaFunction(perMinutePublishNewDataForPortX04Lambda)],
    });
