...
export class PerMinutePublishNewDataForPortX07Lambda  extends Construct {
  constructor(scope: Construct, id: string, api: GraphQLApi, db: TimestreamDatabase, role: LambdaRole) {
    super(scope, id);

    
     
    const perMinutePublishNewDataForPortX07Lambda = new lambda.Function(this, "perMinutePublishNewDataForPortX07Lambda", {
      runtime: lambda.Runtime.NODEJS_16_X,
      handler: "perMinutePublishNewDataForPort-x07.handler",
    ...
    const lambdaDsPerMinutePublishNewDataForPortX07Lambda= api.graphQLApi.addLambdaDataSource(
      "lambdaDsPerMinutePublishNewDataForPortX07Lambda",
      perMinutePublishNewDataForPortX07Lambda
    );
  
    // Resolver for Mutation to publish new data
    lambdaDsPerMinutePublishNewDataForPortX07Lambda.createResolver("createResolverMutationlambdaDsPerMinutePublishNewDataForPortX07", {
      typeName: "Mutation",
      fieldName: "publishNewDataForPortx07",
    ...
    perMinutePublishNewDataForPortX07Lambda.addPermission("EventBridgeInvoke", {
    ...
    new events.Rule(this, "rulePerMinutePublishNewDataForPortX07Lambda", {
      schedule: events.Schedule.rate(cdk.Duration.minutes(1)),
      targets: [new targets.LambdaFunction(perMinutePublishNewDataForPortX07Lambda)],
    });
