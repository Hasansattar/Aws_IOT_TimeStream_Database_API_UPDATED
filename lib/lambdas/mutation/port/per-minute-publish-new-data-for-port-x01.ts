...
export class PerMinutePublishNewDataForPortX01Lambda  extends Construct {
  constructor(scope: Construct, id: string, api: GraphQLApi, db: TimestreamDatabase, role: LambdaRole) {
    super(scope, id);

    const perMinutePublishNewDataForPortX01Lambda = new lambda.Function(this, "perMinutePublishNewDataForPortX01Lambda", {
      runtime: lambda.Runtime.NODEJS_16_X,
      handler: "perMinutePublishNewDataForPort-x01.handler",
    ...
    const lambdaDsPerMinutePublishNewDataForPortX01Lambda= api.graphQLApi.addLambdaDataSource(
      "lambdaDsPerMinutePublishNewDataForPortX01Lambda",
      perMinutePublishNewDataForPortX01Lambda
    );
  
    // Resolver for Mutation to publish new data
    lambdaDsPerMinutePublishNewDataForPortX01Lambda.createResolver("createResolverMutationlambdaDsPerMinutePublishNewDataForPortX01", {
      typeName: "Mutation",
      fieldName: "publishNewDataForPortx01",
    ...
    perMinutePublishNewDataForPortX01Lambda.addPermission("EventBridgeInvoke", {
    ...
    new events.Rule(this, "rulePerMinutePublishNewDataForPortX01Lambda", {
      schedule: events.Schedule.rate(cdk.Duration.minutes(1)),
      targets: [new targets.LambdaFunction(perMinutePublishNewDataForPortX01Lambda)],
    });
