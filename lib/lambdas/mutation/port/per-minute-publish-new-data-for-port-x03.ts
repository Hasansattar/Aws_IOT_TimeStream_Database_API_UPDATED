...
export class PerMinutePublishNewDataForPortX03Lambda  extends Construct {
  constructor(scope: Construct, id: string, api: GraphQLApi, db: TimestreamDatabase, role: LambdaRole) {
    super(scope, id);

    const perMinutePublishNewDataForPortX03Lambda = new lambda.Function(this, "perMinutePublishNewDataForPortX03Lambda", {
      runtime: lambda.Runtime.NODEJS_16_X,
      handler: "perMinutePublishNewDataForPort-x03.handler",
    ...
    const lambdaDsPerMinutePublishNewDataForPortX03Lambda= api.graphQLApi.addLambdaDataSource(
      "lambdaDsPerMinutePublishNewDataForPortX03Lambda",
      perMinutePublishNewDataForPortX03Lambda
    );
  
    // Resolver for Mutation to publish new data
    lambdaDsPerMinutePublishNewDataForPortX03Lambda.createResolver("createResolverMutationlambdaDsPerMinutePublishNewDataForPortX03", {
      typeName: "Mutation",
      fieldName: "publishNewDataForPortx03",
    ...
    perMinutePublishNewDataForPortX03Lambda.addPermission("EventBridgeInvoke", {
    ...
    new events.Rule(this, "rulePerMinutePublishNewDataForPortX03Lambda", {
      schedule: events.Schedule.rate(cdk.Duration.minutes(1)),
      targets: [new targets.LambdaFunction(perMinutePublishNewDataForPortX03Lambda)],
    });
