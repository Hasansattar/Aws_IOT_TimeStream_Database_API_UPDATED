...
export class PerMinutePublishNewDataForSenorFlowLambda  extends Construct {
  constructor(scope: Construct, id: string, api: GraphQLApi, db: TimestreamDatabase, role: LambdaRole) {
    super(scope, id);
    


    const perMinutePublishNewDataForSensorFlowLambda = new lambda.Function(this, "perMinutePublishNewDataForSensorFlowLambda", {
      runtime: lambda.Runtime.NODEJS_16_X,
      handler: "perMinutePublishNewDataForSensorFlow.handler",
    ...
    const lambdaDsPerMinutePublishNewDataForSensorFlowLambda= api.graphQLApi.addLambdaDataSource(
      "lambdaDsPerMinutePublishNewDataForSensorFlowLambda",
      perMinutePublishNewDataForSensorFlowLambda
    );
  
    // Resolver for Mutation to publish new data
    lambdaDsPerMinutePublishNewDataForSensorFlowLambda.createResolver("createResolverMutationlambdaDsPerMinutePublishNewDataForSensorFlow", {
      typeName: "Mutation",
      fieldName: "publishNewDataForSensorFlow",
    ...
    perMinutePublishNewDataForSensorFlowLambda.addPermission("EventBridgeInvoke", {
    ...
    new events.Rule(this, "rulePerMinutePublishNewDataForSensorFlowLambda", {
      schedule: events.Schedule.rate(cdk.Duration.minutes(1)),
      targets: [new targets.LambdaFunction(perMinutePublishNewDataForSensorFlowLambda)],
    });
