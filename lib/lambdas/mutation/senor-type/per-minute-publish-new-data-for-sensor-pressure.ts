...
export class PerMinutePublishNewDataForSenorPressureLambda  extends Construct {
  constructor(scope: Construct, id: string, api: GraphQLApi, db: TimestreamDatabase, role: LambdaRole) {
    super(scope, id);
    

    const perMinutePublishNewDataForSensorPressureLambda = new lambda.Function(this, "perMinutePublishNewDataForSensorPressureLambda", {
      runtime: lambda.Runtime.NODEJS_16_X,
      handler: "perMinutePublishNewDataForSensorPressure.handler",
    ...
    const lambdaDsPerMinutePublishNewDataForSensorPressureLambda= api.graphQLApi.addLambdaDataSource(
      "lambdaDsPerMinutePublishNewDataForSensorPressureLambda",
      perMinutePublishNewDataForSensorPressureLambda
    );
  
    // Resolver for Mutation to publish new data
    lambdaDsPerMinutePublishNewDataForSensorPressureLambda.createResolver("createResolverMutationlambdaDsPerMinutePublishNewDataForSensorPressure", {
      typeName: "Mutation",
      fieldName: "publishNewDataForSensorPressure",
    ...
    perMinutePublishNewDataForSensorPressureLambda.addPermission("EventBridgeInvoke", {
    ...
    new events.Rule(this, "rulePerMinutePublishNewDataForSensorPressureLambda", {
      schedule: events.Schedule.rate(cdk.Duration.minutes(1)),
      targets: [new targets.LambdaFunction(perMinutePublishNewDataForSensorPressureLambda)],
    });
