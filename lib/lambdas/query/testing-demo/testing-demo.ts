...
export interface  QueryHistoryTestingProps  {
  config: any; // You can replace 'any' with a more specific type if you have one
}



export class QueryHistoryTestingLambda extends Construct {
  constructor(scope: Construct, id: string, api: GraphQLApi,db: TimestreamDatabase, role: LambdaRole,props:QueryHistoryTestingProps) {
    super(scope, id);
    ...
    const queryHistoryTestingLambda = new lambda.Function(this, `${config.stage}-QueryHistoryTestingLambda`, {
      runtime: lambda.Runtime.NODEJS_16_X,
      handler: "handler.handler",
      code: lambda.Code.fromAsset("lib/lambdas/query/testing-demo/asset"),
      environment: {
