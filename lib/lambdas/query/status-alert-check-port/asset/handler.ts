...
// PREVIOUS QUERY FOR  STATUS PORT STATUS
...
// NEW QUERY FOR STATUS PORT STATUS

const queryStringStatusAlertPorts =`WITH DataReceivedPorts AS (
...
AllPorts AS (
    SELECT 'x01' AS port
    UNION ALL SELECT 'x04'
    UNION ALL SELECT 'x05'
    UNION ALL SELECT 'x07'
    UNION ALL SELECT 'x08'
    UNION ALL SELECT 'mo1'
    UNION ALL SELECT 'x03'
    -- Add other ports as necessary
)

-- Final selection to determine connection status
SELECT ap.port, 
       CASE 
           WHEN drp.port IS NOT NULL THEN 'Connected'
           ELSE 'Not Connected'
       END AS connection_status
FROM AllPorts ap
LEFT JOIN DataReceivedPorts drp ON ap.port = drp.port
ORDER BY ap.port
`;
...
// ======================END WATER PRODUCTION TOTILIZER==============================
  ...
  try {
    ...
    return statusAlertPortForplant;
