...
export class QueryStatusOperationLambda extends Construct {
  constructor(scope: Construct, id: string, api: GraphQLApi, dynamostatuscheck:DynamoDatabaseStatusCheck ,dynamo:DynamoDatabase,db: TimestreamDatabase, role: LambdaRole,props:QueryStatusOperationLambdaProps) {
    super(scope, id);
    ...
    const queryStatusOperationLambda = new lambda.Function(this, `${config.stage}-QueryStatusOperationLambda`, {
      ...
      code: lambda.Code.fromAsset("lib/lambdas/query/status-operation/asset"),
      environment: {
    ...
    dynamostatuscheck.DynamoTable.grantFullAccess(queryStatusOperationLambda);
